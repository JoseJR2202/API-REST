...
exports.insertProveedor = async (prov) => {
    ...
    try {
        ...
        const response = (await client.query(querys_1.querys_proveedor.SIGN_UP_proveedor, [nombre, contacto, direccion])).rows[0];
        const proveedor = {
...
exports.getProveedores = async () => {
    const client = await pool.connect();
    try {
        const response = (await client.query(querys_1.querys_proveedor.GET_proveedor)).rows;
...
exports.updateProveedor = async ({ prov, ide }) => {
    ...
    try {
        await client.query('BEGIN');
        const response = (await client.query(querys_1.querys_proveedor.UPDATE_proveedor, [nombre, contacto, direccion, ide])).rows[0];
        const empleados = {
            id: response.id_proveedor,
            nombre: response.nombre,
            contacto: response.contacto,
...
exports.deleteProveedor = async (id) => {
    ...
    try {
        await client.query('BEGIN');
        const response = (await client.query(querys_1.querys_proveedor.DELETE_proveedor, [id])).rowCount > 0;
        await client.query('COMMIT');
        return response;
    }
    catch (e) {
